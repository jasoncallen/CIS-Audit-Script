...
print_header "1.5.1.5 - Ensure SELinux mode is enforcing"
if [ "$mode" == "enforcing" ]; then
    log_pass "SELinux mode is enforcing"
else
    log_fail "SELinux mode is NOT enforcing (Found: $mode)"
fi

# 1.5.1.6 - Ensure no unconfined services exist (Basic check)

print_header "5.2.1.2 - Ensure auditing is enabled at boot"
if grep -q "audit=1" /etc/default/grub; then
    log_pass "audit=1 is set in GRUB config"
else
    log_fail "audit=1 is NOT found in GRUB config"
fi

# 5.2.1.3 - Ensure audit_backlog_limit is sufficient

print_header "5.2.1.3 - Ensure audit_backlog_limit is sufficient"
if grep -q "audit_backlog_limit=" /etc/default/grub; then
    log_pass "audit_backlog_limit is defined"
else
    log_fail "audit_backlog_limit is NOT set in GRUB config"
fi
...
if grep -q "^max_log_file_action\s*=\s*keep_logs" /etc/audit/auditd.conf; then
    ...
    log_fail "audit log retention policy is not set to keep_logs"
fi

# 5.2.2.3 - Ensure system is disabled when audit logs are full

print_header "5.2.3.20 - Ensure audit rules are immutable"
if tail -1 /etc/audit/rules.d/*.rules 2>/dev/null | grep -q "^-e 2"; then
    log_pass "Audit rules are set to immutable"
else
    log_fail "Audit rules are NOT set to immutable (-e 2)"
fi

# 5.3 - Configure File Integrity (AIDE)