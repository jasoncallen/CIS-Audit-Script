...
# Section 1: Hardware (Level 2)
log "Section 1: Hardware (Level 2)"

# 1.8 (L2) Host must configure Intel VT-d or AMD-Vi IOMMU and enable passthrough for approved devices only (Manual)
manual "1.8 Ensure VT-d/AMD-Vi IOMMU is enabled and passthrough is configured for approved devices only"


# Section 2: Base (Level 2)
log "Section 2: Base (Level 2)"

# 2.12 (L2) Host must restrict read-only VIB installation (Automated)
vib_policy=$(esxcli software acceptance get)
if [[ "$vib_policy" == "VMwareCertified" ]]; then
    pass "2.12 Read-only VIB installation restricted to VMwareCertified"
else
    fail "2.12 VIB acceptance level is not sufficiently restricted: $vib_policy"
fi

# 2.13 (L2) Host must disable untrusted VIB execution (Manual)
manual "2.13 Ensure execution of untrusted VIBs is disabled"


# Section 3: Management (Level 2)
log "Section 3: Management (Level 2)"

# 3.21 (L2) Host must enable Strict Lockdown Mode (Manual)
manual "3.21 Ensure Strict Lockdown Mode is enabled"

# 3.23 (L2) Host must restrict shell access to DCUI.Access users (Manual)
manual "3.23 Ensure only users in DCUI.Access list can access the shell"


# Section 4: Logging (Level 2)
log "Section 4: Logging (Level 2)"

# 4.12 (L2) Host must use TLS to protect all log transmissions (Manual)
manual "4.12 Ensure TLS is used to protect all log transmissions"

# 4.13 (L2) Host must use certificates issued by a trusted CA for remote logging (Manual)
manual "4.13 Ensure certificates from trusted CA are used for remote logging"

# 4.14 (L2) Host must store one year of logs (Manual)
manual "4.14 Ensure logs are retained for at least one year"


# Section 5: Network (Level 2)
log "Section 5: Network (Level 2)"

# 5.5 (L2) Host must use VLANs to logically segment traffic (Manual)
manual "5.5 Ensure VLANs are used to segment network traffic logically"


# Section 6: Services (Level 2)
log "Section 6: Services (Level 2)"

# 6.14 (L2) Host must disable SSH access to all users except when needed for diagnostics (Manual)
manual "6.14 Ensure SSH is disabled for all users except when required for diagnostics"


# Section 7: Authentication (Level 2)
log "Section 7: Authentication (Level 2)"

# 7.7 (L2) Host must enforce multifactor authentication (Manual)
manual "7.7 Ensure multifactor authentication is enforced for host access"


# Section 8: Users (Level 2)
log "Section 8: Users (Level 2)"

# 8.6 (L2) Host must restrict login times for named accounts (Manual)
manual "8.6 Ensure login times are restricted for named accounts"


# Section 9: Virtual Machines (Level 2)
log "Section 9: Virtual Machines (Level 2)"

# 9.7 (L2) VM must use virtual Trusted Platform Module (vTPM) (Manual)
manual "9.7 Ensure virtual TPM (vTPM) is enabled for VMs"

# 9.8 (L2) VM must enable Microsoft Virtualization Based Security (VBS) when supported (Manual)
manual "9.8 Ensure VBS is enabled for VMs when supported"


# Section 10: Auditing (Level 2)
log "Section 10: Auditing (Level 2)"

# 10.5 (L2) Host must audit use of external devices (Manual)
manual "10.5 Ensure usage of external devices is audited"

# 10.6 (L2) Host must audit all command and script executions (Manual)
manual "10.6 Ensure all command and script executions are audited"

# 10.7 (L2) Host must audit access to privileged functions (Manual)
manual "10.7 Ensure access to privileged functions is audited"

# 10.8 (L2) Host must audit modifications to audit configurations (Manual)
manual "10.8 Ensure modifications to audit configurations are audited"
